...
<div class="content-wrapper"> <?php //print_r($this->session->userdata()); die;?>
        ...
        <div class="box box-info">
			<?php
				if(empty($data['hits']['hits'][0]['_source']['position_file'])){
					$rand = str_split(rand(10,100)); //print_r(implode("/", $rand)); die;
					$implode_rand = implode("/", str_split(rand(10,100)));
					$position_file = URL_IMG_DATAKL.$implode_rand;
					$nn = 2;
				} else {
			...
			?>
          <form method="POST" action="/datakl/added/<?= $data['hits']['hits'][0]['_id'] ?>?url=<?= $position_file ?>&nn=<?= $nn ?>" enctype="multipart/form-data">
			<?php //echo form_open('/datakl/added/'.$data['hits']['hits'][0]['_id']); ?>
					...
					<div class="row">
						...
						<div class="col-md-6"> <?php //print_r($datakb); die; ?>
							<div class="form-group">
								...
								<select class="form-control number_of_kl_class" name="number_of_kb">
									...
									<?php
										if(!empty($datakb)){
											foreach($datakb['hits']['hits'] as $k => $v){
												...
												if(!empty($data['hits']['hits'][0]['_source']['number_of_kb'])){
													if($v['_source']['number_of_kb'] == $data['hits']['hits'][0]['_source']['number_of_kb']){
														$selected = "selected";
							...
							<div class="form-group" >
								...
								<input type="text" name="project_name" class="form-control" placeholder="Project Name" value="<?php echo @$data['hits']['hits'][0]['_source']['project_name_kb']; ?>">
							</div>

							<div class="form-group" >
								<label for="Project Name">CC Number</label>
								<?= @$validation_cc_number ?>
								<input type="text" name="cc_number" class="form-control" placeholder="CC Number" value="<?php echo @$data['hits']['hits'][0]['_source']['client_contract_number']; ?>">
							</div>

							<div class="form-group" >
								<label for="Project Name">KL Number</label>
								<?= @$validation_telkom_number ?>
								<input type="text" name="telkom_number" class="form-control" placeholder="KL Number" value="<?php echo @$data['hits']['hits'][0]['_source']['contract_number']; ?>">
							</div>

							<div class="form-group" >
								<label for="Project Name">RAK Number</label>
								<?= @$validation_rak_number ?>
								<input type="text" name="number_of_rack" class="form-control" placeholder="RAK Number" value="<?php echo @$data['hits']['hits'][0]['_source']['number_of_rack'] ?>">
							</div>

							<div class="form-group">
								<label for="merk">SPK</label>
								<?= @$validation_spk ?>
								<input type="text" name="spk" class="form-control" placeholder="SPK" value="<?= @$data['hits']['hits'][0]['_source']['spk'] ?>">
							</div> 
									...
									<?php
										if(!empty($data_contract_doc_status)){
											foreach($data_contract_doc_status as $k => $v){
												...
												if(!empty($data['hits']['hits'][0]['_source']['contract_doc_status'])){
													if($v['id_contract_doc_status'] == $data['hits']['hits'][0]['_source']['contract_doc_status']){
														$selected = "selected";
													}
												}
										?>
						...
						<div class="col-md-6">
									...
									<?php
										if(!empty($data_segment)){
											foreach($data_segment as $k => $v){
												...
												if(!empty($data['hits']['hits'][0]['_source']['contract_doc_status'])){
													if($v['id_segment'] == $data['hits']['hits'][0]['_source']['contract_doc_status']){
														$selected = "selected";
													}
												}
										?>
							...
							<a target=_blank href=/datakl/download?url=<?= $data['hits']['hits'][0]['_source']['position_file']?>><img width="10%" src="/assets/dist/img/documents-icon.png"></a>
